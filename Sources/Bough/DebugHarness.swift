...
//   e.g.  .build/debug/Bough --preview approval
//
// Scenarios:
...
enum PreviewScenario: String, CaseIterable {
    ...
    case stress
    // AirDrop demo scenarios
    case airdropMagnet = "airdrop-magnet"
    case airdropDropzone = "airdrop-dropzone"
    case airdropFileReady = "airdrop-file-ready"
    case airdropURLReady = "airdrop-url-ready"
    case airdropTextConfirm = "airdrop-text-confirm"
    case airdropUnavailable = "airdrop-unavailable"
    case airdropFailed = "airdrop-failed"
    case airdropCleanupError = "airdrop-cleanup-error"
    case airdropCompletionOverlay = "airdrop-completion-overlay"
    case airdropApprovalBlocked = "airdrop-approval-blocked"
    case airdropQuestionBlocked = "airdrop-question-blocked"
    case airdropDisabled = "airdrop-disabled"
}

enum AirDropDemoScenario: String, CaseIterable, Identifiable {
    case magnet
    case dropzone
    case fileReady
    case urlReady
    ...
    var title: String {
        ...
        case .fileReady: return "file-ready"
        case .urlReady: return "url-ready"
        case .textConfirm: return "text-confirm"
        case .unavailable: return "unavailable"
        case .failed: return "failed"
        case .cleanupError: return "cleanup-error"
        case .completionOverlay: return "completion-overlay"
        case .approvalBlocked: return "approval-blocked"
        case .questionBlocked: return "question-blocked"
    ...
    var displayTitle: String {
        switch self {
        case .magnet: return "磁吸提示"
        case .dropzone: return "拖拽区"
        case .fileReady: return "文件已准备"
        case .urlReady: return "链接已准备"
        case .textConfirm: return "确认文字"
        case .unavailable: return "不可用"
        case .failed: return "失败"
        case .cleanupError: return "清理失败"
        case .completionOverlay: return "完成覆盖层"
        case .approvalBlocked: return "权限阻塞"
        case .questionBlocked: return "问题阻塞"
        case .disabled: return "功能关闭"
        }
    }
}
...
enum DebugHarness {
    ...
    static func apply(_ scenario: PreviewScenario, to appState: AppState) {
        ...
        case .airdropMagnet: applyAirDropDemo(.magnet, to: appState)
        case .airdropDropzone: applyAirDropDemo(.dropzone, to: appState)
        case .airdropFileReady: applyAirDropDemo(.fileReady, to: appState)
        case .airdropURLReady: applyAirDropDemo(.urlReady, to: appState)
        case .airdropTextConfirm: applyAirDropDemo(.textConfirm, to: appState)
        case .airdropUnavailable: applyAirDropDemo(.unavailable, to: appState)
        case .airdropFailed: applyAirDropDemo(.failed, to: appState)
        case .airdropCleanupError: applyAirDropDemo(.cleanupError, to: appState)
        case .airdropCompletionOverlay: applyAirDropDemo(.completionOverlay, to: appState)
        case .airdropApprovalBlocked: applyAirDropDemo(.approvalBlocked, to: appState)
        case .airdropQuestionBlocked: applyAirDropDemo(.questionBlocked, to: appState)
        case .airdropDisabled: applyAirDropDemo(.disabled, to: appState)
        }
        finalizePreviewState(appState)
    }
    ...
    static func applyAirDropDemo(_ scenario: AirDropDemoScenario, to state: AppState) {
        ...
        state.clearAirDropDragIndicators()
        state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        state.airDropIncludeText = false

        switch scenario {
        case .magnet:
            state.surface = .collapsed
            state.airDropMagnetPreheating = true
        case .dropzone:
            state.showAirDropDropZone(highlighted: true)
        case .fileReady:
            state.beginAirDrop(
                payload: AirDropPasteboardPayload(fileURLs: [URL(fileURLWithPath: "/tmp/Bough Demo File.txt")]),
                source: .preview
            )
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .urlReady:
            state.beginAirDrop(
                payload: AirDropPasteboardPayload(remoteURLs: [URL(string: "https://bough.app/demo")!]),
                source: .preview
            )
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .textConfirm:
            state.beginAirDrop(
                payload: AirDropPasteboardPayload(text: "Bough demo selected text\nSecond preview line\nHidden third line"),
                source: .preview
            )
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .unavailable:
        ...
        case .completionOverlay:
            ...
            state.showAirDropDropZone()
            state.airDropPreviewOverlayPhase = .dropZone
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .approvalBlocked:
            applyApproval(to: state)
            state.showAirDropDropZone()
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .questionBlocked:
            applyQuestion(to: state)
            state.showAirDropDropZone()
            state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        case .disabled:
            UserDefaults.standard.set(false, forKey: SettingsKey.airDropEnabled)
            state.surface = .sessionList
            state.airDropState = .idle
        }
        state.airDropPreviewScenarioName = "airdrop-\(scenario.title)"
        finalizePreviewState(state)
    }

    private static func finalizePreviewState(_ state: AppState) {
    ...
    private static func applyApproval(to state: AppState) {
        ...
        state.previewPermissionEvent = makePreviewPermissionEvent(
    ...
    private static func makePreviewPermissionEvent(
        sessionId: String,
        toolName: String,
