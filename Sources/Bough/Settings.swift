...
enum SettingsNotification {
    ...
    /// connectivity indicator on the next render.
    static let claudeCodeStatusLineDidChange =
        Notification.Name("com.dgpisces.bough.ClaudeCodeStatusLineDidChange")
}

enum AppVersion {
    /// Update this each release. Used as fallback when Info.plist is unavailable (debug builds).
    static let fallback = "v1.0.0-rc.1"
    ...
    static var current: String {
        ...
        return Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? fallback
    }
}
...
enum CompactBarPriority: String, CaseIterable, Identifiable {
    case aiActivity = "aiActivity"
    case music = "music"
    ...
    var localizedKey: String {
        switch self {
        case .aiActivity:
            return "compact_priority_ai_activity"
        case .music:
            return "compact_priority_music"
        }
    }

    static func normalized(_ rawValue: String?) -> CompactBarPriority {
    ...
    static func normalizedRawValue(_ rawValue: String?) -> String {
        normalized(rawValue).rawValue
    }

    static func visiblePriority(rawValue: String?, musicControlsEnabled: Bool) -> CompactBarPriority {
        let priority = normalized(rawValue)
        if priority == .music && !musicControlsEnabled {
            return .aiActivity
        }
        return priority
    }

    static func resolvedSource(rawValue: String?, musicControlsEnabled: Bool, aiAvailable: Bool, musicAvailable: Bool) -> CompactBarPriority? {
        guard aiAvailable || (musicControlsEnabled && musicAvailable) else {
            return nil
        }

        switch visiblePriority(rawValue: rawValue, musicControlsEnabled: musicControlsEnabled) {
        case .aiActivity:
            return aiAvailable ? .aiActivity : (musicAvailable ? .music : nil)
        case .music:
            return musicAvailable ? .music : (aiAvailable ? .aiActivity : nil)
        }
    }
}
...
enum SettingsKey {
    ...
    // General - Behavior
    static let airDropEnabled = "airDropEnabled"
    static let airDropDemoScenariosEnabled = "airDropDemoScenariosEnabled"
    ...
    static let showMusicControls = "showMusicControls"
    static let compactBarPriority = "compactBarPriority"
    ...
    // Usage provider persistence
    static let usageSelectedProvider = "usageSelectedProvider"
    static func usageDisplayEnabled(_ toolRawValue: String) -> String { "usageDisplayEnabled.\(toolRawValue)" }
    static func usageStatisticsEnabled(_ toolRawValue: String) -> String { "usageStatisticsEnabled.\(toolRawValue)" }
    static let codexHooksDisabledNoticeDismissed = "codexHooksDisabledNoticeDismissed"
    static let usageContinuityWriterOwner = "usageContinuityWriterOwner"
    static let usageMonitorRestoreAfterCodingSessionsEnabled = "usageMonitorRestoreAfterCodingSessionsEnabled"
    ...
    static func codexCLIOutdatedDismissalKey(forDetectedVersion version: String) -> String {
        let sanitized = version.unicodeScalars.map { scalar -> Character in
        ...
        let suffix = sanitized.isEmpty ? "unknown" : sanitized
        return "bough.dismissedCodexCLIOutdated.\(suffix)"
    }
}
...
struct SettingsDefaults {
    ...
    static let airDropDemoScenariosEnabled = false
    ...
    static let showMusicControls = true
    static let compactBarPriority = CompactBarPriority.aiActivity.rawValue
    ...
    static let sessionGroupingMode = "status"
    ...
    static let usageSelectedProvider = "codex"
}

enum SessionGroupingMode {
...
enum CodingSessionsSettings {
    static let titleLocalizationKey = "coding_sessions"
    static let descriptionLocalizationKey = "coding_sessions_desc"

    static func isEnabled(defaults: UserDefaults = .standard) -> Bool {
        guard defaults.object(forKey: SettingsKey.codingSessionsEnabled) != nil else {
            return SettingsDefaults.codingSessionsEnabled
        }
        return defaults.bool(forKey: SettingsKey.codingSessionsEnabled)
    }

    static func setEnabled(_ isEnabled: Bool, defaults: UserDefaults = .standard) {
        defaults.set(isEnabled, forKey: SettingsKey.codingSessionsEnabled)
    }
}
...
enum AirDropSettings {
    static func isEnabled(defaults: UserDefaults = .standard) -> Bool {
        guard defaults.object(forKey: SettingsKey.airDropEnabled) != nil else {
            return SettingsDefaults.airDropEnabled
        }
        return defaults.bool(forKey: SettingsKey.airDropEnabled)
    }
}
...
class SettingsManager {
    ...
    private init() {
        ...
        defaults.register(defaults: [
            ...
            SettingsKey.airDropEnabled: SettingsDefaults.airDropEnabled,
            SettingsKey.airDropDemoScenariosEnabled: SettingsDefaults.airDropDemoScenariosEnabled,
            ...
            SettingsKey.showMusicControls: SettingsDefaults.showMusicControls,
            SettingsKey.compactBarPriority: SettingsDefaults.compactBarPriority,
            ...
            SettingsKey.usageSelectedProvider: SettingsDefaults.usageSelectedProvider,
            SettingsKey.codexHooksDisabledNoticeDismissed: false,
        ])
    }
    ...
    var codingSessionsEnabled: Bool {
        get { CodingSessionsSettings.isEnabled(defaults: defaults) }
        set { CodingSessionsSettings.setEnabled(newValue, defaults: defaults) }
    }
    ...
    var airDropEnabled: Bool {
        get { AirDropSettings.isEnabled(defaults: defaults) }
        set { defaults.set(newValue, forKey: SettingsKey.airDropEnabled) }
    }
    ...
    var showMusicControls: Bool {
        get { defaults.bool(forKey: SettingsKey.showMusicControls) }
        set { defaults.set(newValue, forKey: SettingsKey.showMusicControls) }
    }

    var compactBarPriority: CompactBarPriority {
        get { CompactBarPriority.normalized(defaults.string(forKey: SettingsKey.compactBarPriority)) }
        set { defaults.set(newValue.rawValue, forKey: SettingsKey.compactBarPriority) }
    }
    ...
    var sessionGroupingMode: String {
        get { SessionGroupingMode.normalized(defaults.string(forKey: SettingsKey.sessionGroupingMode)) }
        set { defaults.set(SessionGroupingMode.normalized(newValue), forKey: SettingsKey.sessionGroupingMode) }
    }
