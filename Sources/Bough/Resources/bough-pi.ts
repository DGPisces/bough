/**
 * @fileoverview Bough Integration Extension.
 *
 * Bridges the running pi session to the Bough macOS floating-window app
 * (https://github.com/DGPisces/bough) by forwarding lifecycle events over
 * the Unix domain socket Bough listens on.
 *
 * Architecture:
 ...
 *
 * Requirements:
 *   - Bough.app running on the same machine
 *   - "pi" added to Bough's supported sources (see PR instructions)
 */
...
/** Unix socket path Bough listens on (user-scoped). */
const SOCKET_PATH = `/tmp/bough-${getuid()}.sock`;
 ...
 */
const BRIDGE_PATH = `${homedir()}/.bough/bough-bridge`;

/** Environment variable keys forwarded to Bough for terminal detection. */
const ENV_KEYS = [
  "TERM_PROGRAM",
  "ITERM_SESSION_ID",
  "TERM_SESSION_ID",
...
/**
 * Sends a JSON payload to the Bough socket (fire-and-forget).
 * Returns `false` silently when Bough is not running.
 *
...
/**
 * Sends a JSON payload via the bridge binary and waits for Bough's response.
...
/**
 * Builds the base fields required on every Bough event payload.
 *
...
export default function boughExtension(pi: ExtensionAPI) {
  /** TTY path detected once at startup. */
  const tty = detectTty();

  /**
   * Session IDs for which a blocking PermissionRequest is currently in flight.
   * Non-lifecycle events for these sessions are suppressed to prevent Bough's
