...
private let log = Logger(subsystem: "com.dgpisces.bough", category: "Panel")
...
class PanelWindowController: NSObject, NSWindowDelegate {
    ...
    private var airDropDragOverlayController: AirDropDragOverlayController?
    ...
    func showPanel() {
        ...
        sessionObservationTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                withObservationTracking {
                    ...
                    _ = self?.appState.musicStore.snapshot?.hasCurrentVisibleTrack
                } onChange: {
        ...
        observeMusicActivityChanges()
        refreshMusicPresentationNeededFromSettings()
        configureAutoScreenPolling()
        configureAirDropDragOverlay()
        ...
        globalClickMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.leftMouseDown, .rightMouseDown]) { [weak self] event in
            Task { @MainActor in
                ...
                case .approvalCard, .questionCard, .airDrop: return
                default: break
                }
        ...
        let observer = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor [self] in
                ...
                if newChoice != self.lastDisplayChoice {
                    ...
                    self.refreshMusicPresentationNeededFromSettings()
                    self.updateVisibility()
                    self.updatePosition()
                }
            }
    ...
    private func observeMusicActivityChanges() {
        musicActivityObserver = NotificationCenter.default.addObserver(
    ...
    private func refreshMusicPresentationNeededFromSettings() {
        appState.musicStore.setPresentationNeeded(
            MusicPanelActivityPolicy.presentationNeeded(
                musicControlsEnabled: SettingsManager.shared.showMusicControls,
                hideWhenIdle: SettingsManager.shared.hideWhenNoSession,
    ...
    private func configureAirDropDragOverlay() {
        airDropDragOverlayController?.stop()
        airDropDragOverlayController = AirDropDragOverlayController(
            appState: appState,
            ...
            notchHeightProvider: { [weak self] in
                guard let self else { return ScreenDetector.notchHeight }
                return ScreenDetector.topBarHeight(for: self.chosenScreen())
            },
            panelVisibilityProvider: { [weak self] in
                guard let self, let panel = self.panel, panel.isVisible else { return false }
                return !(SettingsManager.shared.hideInFullscreen && self.fullscreenLatch)
            }
        )
        airDropDragOverlayController?.start()
    }
    ...
    private func updateVisibility() {
        ...
        let hasVisibleMusicActivity = MusicPanelActivityPolicy.hasVisibleMusicActivity(
            snapshot: appState.musicStore.snapshot,
            musicControlsEnabled: settings.showMusicControls
        )
        let shouldShowPanel = MusicPanelActivityPolicy.shouldShowPanel(
            hideWhenIdle: settings.hideWhenNoSession,
            codingSessionsEnabled: CodingSessionsSettings.isEnabled(),
    ...
    deinit {
        ...
        if let observer = musicActivityObserver {
            NotificationCenter.default.removeObserver(observer)
        }
        ...
        if let airDropDragOverlayController {
