...
public final class CodexAppServerClient: @unchecked Sendable {
    ...
    public init(
        ...
        self.ioQueue = DispatchQueue(label: "com.dgpisces.bough.codex-app-server-io")
    }
    ...
    var activeReadabilityHandlerCountForTesting: Int {
        lock.lock(); defer { lock.unlock() }
        return [stdoutReadHandle, stderrReadHandle].reduce(0) { count, handle in
            count + (handle?.readabilityHandler == nil ? 0 : 1)
        }
    }
    ...
    public func start() throws {
        ...
        do {
            ...
            Self.closeProcessHandles(
                DetachedProcessHandles(
                    process: nil,
                    stdin: stdinWriter,
                    stdout: stdoutReader,
                    stderr: stderrReader
    ...
    private struct DetachedProcessHandles {
        let process: Process?
    ...
    private func detachProcessHandlesLocked() -> DetachedProcessHandles {
        let handles = DetachedProcessHandles(
            process: process,
            stdin: stdinHandle,
            stdout: stdoutReadHandle,
            stderr: stderrReadHandle
        )
        process = nil
        stdinHandle = nil
    ...
    private static func closeProcessHandles(_ handles: DetachedProcessHandles, terminateProcess: Bool) {
